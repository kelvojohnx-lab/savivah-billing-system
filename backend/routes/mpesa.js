...
router.post('/callback', async (req, res) => {
  ...
  try {
    ...
    voucher.mpesaRef = payment.mpesaRef;
    const tx = createTransaction(voucher, 'mpesa', payment.mpesaRef);

    // Save mpesa ref to voucher
    db.update(data => {
    ...
    console.log(`✅ M-Pesa payment confirmed: ${payment.mpesaRef} → voucher ${voucher.code}`);
  } catch (err) {
    console.error('Callback processing error:', err.message);
  }
});

// Manual STK validation (for sandbox testing)
